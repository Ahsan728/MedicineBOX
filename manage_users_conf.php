                    ...
                    if (!empty($_POST['gender'])) {
                        ...
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            if (!$row = mysqli_fetch_assoc($resultl)) {
                                ...
                                $sql="UPDATE users SET username=?, serialnumber=?, medicine=?, gender=?, user_date=CURDATE() WHERE fingerprint_select=1 AND device_uid=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    mysqli_stmt_bind_param($result, "sdsss", $Uname, $Number, $Medicine, $Gender, $dev_uid );
                                    mysqli_stmt_execute($result);
                        ...
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            if (!$row = mysqli_fetch_assoc($resultl)) {                 
                                if (!empty($Uname)) {
                                    ...
                                    $sql="UPDATE users SET username=?, serialnumber=?, medicine=?, gender=? WHERE fingerprint_select=1 AND device_uid=?";
                                    $result = mysqli_stmt_init($conn);
                                    if (!mysqli_stmt_prepare($result, $sql)) {
                                        echo "SQL_Error_select_Fingerprint";
                                        exit();
